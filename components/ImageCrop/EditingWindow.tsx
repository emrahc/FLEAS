...
function EditingWindow() {
  ...
  const [, setImageBounds] = useRecoilState(
    imageBoundsState
  );
  const [, setImageScaleFactor] = useRecoilState(
    imageScaleFactorState
  );
  ...
  const onUpdateCropLayout = (layout) => {
    ...
    if (layout.height != null && layout.width != null) {
      ...
      const editingWindowAspectRatio =
