...
function ImageCropOverlay() {
  ...
  const [fixedAspectRatio] = useRecoilState(
    fixedCropAspectRatioState
  );
  const [lockAspectRatio] = useRecoilState(
    lockAspectRatioState
  );
  const [minimumCropDimensions] = useRecoilState(
    minimumCropDimensionsState
  );
  ...
  const panX = React.useRef(
    new Animated.Value(imageBounds.x)
  );
  const panY = React.useRef(
    new Animated.Value(imageBounds.y)
  );
  ...
  const onOverlayMove = ({
    ...
    if (selectedFrameSection !== "") {
      ...
      if (isMovingSection()) {
        ...
        animatedCropSize.height.setValue(
          cropSize.height + y
        );
      }
    } else {
